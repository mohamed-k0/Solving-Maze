...
class MoveX_Server(Node):
    ...
    def __init__(self):
        ...
        self.heading_pid = PID(heading_kp,heading_ki,heading_kd,max_heading_vel,-max_heading_vel,heading_integral_limit,-heading_integral_limit,heading_deadzone,angles=True)
    ...
    def execute_callback(self, goal):
        try:
            ...
            target = goal.request.forward_distance
            ...
            self.heading_pid.set_target(initial_yaw)
            ...
            while rclpy.ok():
                ...
                heading_correction = self.heading_pid.control(self.yaw, dt)
                ...
                # Feedback
                progress_val = 0.0 if target == 0.0 else (abs(dist_done) / abs(target)) * 100
                feedback.progress = float(progress_val)
